...
Before using this repository, you must ensure your environment meets all required prerequisites. We provide two scripts to automate this process:

1. `bootstrap_env.sh` - Prepares your environment by installing and configuring required dependencies
2. `validate_env.sh` - Validates that your environment meets all requirements

### Initial Setup

```bash
# 1. First, run the bootstrap script to prepare your environment
...
- Ansible (>= 2.9)
- Libvirt (>= 6.0) and QEMU-KVM
- Required system packages and dependencies
- User permissions for KVM and Libvirt
...
- Ansible >= 2.9
- Libvirt >= 6.0
- KVM enabled with nested virtualization

> **Note:** If you make any changes to the system requirements or need to modify the environment configuration, update the `bootstrap_env.sh` script and run the bootstrap/validate process again. All environment requirements are defined in the [system_card.yml](system_card.yml) file.
...
4. [**Setting up a local Red Hat Quay Registry.**](#4-setting-up-a-local-quay-registry) - Deploy a Quay mirror registry with native OpenShift support.
5. [**Setting up a local Harbor Registry.**](#5-setting-up-a-local-harbor-registry) - Deploy a quick Harbor registry for testing.
6. [**Setting up a local JFrog Registry.**](#6-setting-up-a-local-jfrog-container-registry) - Deploy a JFrog registry for testing.
7. [**Deploying KVM Virtual Machines for Container Registries.**](#7-deploying-kvm-virtual-machines-for-container-registries) - Provision KVM virtual machines to host Quay, Harbor, or JFrog Artifactory registries.

## 7. Deploying KVM Virtual Machines for Container Registries

To provision KVM virtual machines to host Quay, Harbor, or JFrog Artifactory registries, you can use the `roles/kvm_provisioner/tasks/main.yml` playbook. This playbook will create VMs based on the specifications defined in the `system_card.yml` file.

Before running the playbook, make sure you have defined the appropriate inventory groups for each registry type in the `inventory` file:

```ini
[quay_vms]
# Add the IP addresses or hostnames of VMs designated for Quay deployment here

[harbor_vms]
# Add the IP addresses or hostnames of VMs designated for Harbor deployment here

[jfrog_vms]
# Add the IP addresses or hostnames of VMs designated for JFrog deployment here
```

To provision VMs for a specific registry, use the `-l` or `--limit` option when invoking the playbook. For example, to create VMs for Quay:

```bash
ansible-playbook -i inventory roles/kvm_provisioner/tasks/main.yml -l quay_vms
```

Similarly, to create VMs for Harbor or JFrog, use:

```bash
ansible-playbook -i inventory roles/kvm_provisioner/tasks/main.yml -l harbor_vms
ansible-playbook -i inventory roles/kvm_provisioner/tasks/main.yml -l jfrog_vms
```

After provisioning the VMs, you can deploy the chosen container registry using the corresponding setup playbook, also using the `-l` option to target the correct VM group. For example, to deploy Quay on the `quay_vms` group:

```bash
ansible-playbook -i inventory playbooks/setup-quay-registry.yml -l quay_vms
```

These steps are integrated into the Molecule testing workflow. The `molecule/default/molecule.yml`, `molecule/harbor/molecule.yml`, and `molecule/jfrog/molecule.yml` files define the platforms and inventory groups for each registry type. When running Molecule tests, the appropriate VMs will be created and targeted based on these configurations.

## System Architecture and Requirements

The system architecture, components, deployment requirements, and environment constraints are documented in the [system_card.yml](system_card.yml) file. This file serves as a versioned blueprint for the OpenShift disconnected deployment system and should be referenced for a comprehensive understanding of the system design.
...
## 4. Setting up a local Quay Registry

In the case you need a Quay registry for your disconnected environment, you can quickly spin one up on any subscribed RHEL system.
...
- Quay registry configuration with OpenShift-ready settings

1. Modify the `inventory` file under the `quay` group to reflect the target host that will run Quay. If you're running this on the same target host then just modify to a localhost type inventory host with `ansible_connection=local ansible_host=localhost`.
2. Configure required variables:
   - quay_hostname: Your Quay server hostname
   - base_domain: Your base domain
   - SSL settings (self-signed or Let's Encrypt)

```bash=
# Run the automation playbook
ansible-playbook -i inventory setup-quay-registry.yml
```

### Post Configuration for Quay Mirroring

After deployment:
1. Access the Quay UI and create your first administrator account
2. Create an Organization for your OpenShift content (e.g., `openshift-mirror`)
3. Create a Robot Account with write access to push content
4. Configure mirroring settings if needed

## 5. Setting up a local Harbor Registry
...
[How to Run the `versions_check.sh` Script](versions_check.md)
```bash
./versions_check.sh
```
