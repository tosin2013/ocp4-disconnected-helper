...
3. [**Setting up Red Hat Quay Mirror Registry.**](#3-setting-up-red-hat-quay-mirror-registry) - Deploy mirror-registry optimized for OpenShift disconnected deployments.
...
## 3. Setting up Red Hat Quay Mirror Registry

Red Hat Quay mirror-registry is a lightweight, single-node container registry optimized for disconnected OpenShift deployments. It provides the official Red Hat supported solution for mirroring OpenShift container images.

**Features:**
- Lightweight deployment (single VM or localhost)
- Built-in PostgreSQL and Redis
- Self-signed or custom SSL certificates
- REST API and Web UI
- Fully supported by Red Hat for OpenShift mirroring

### Quick Start

```bash
# Deploy to a dedicated VM
ansible-playbook -i inventory/ibm-cloud.yml \
  playbooks/setup-mirror-registry.yml \
  -e "target_host=registry" \
  -e "mirror_registry_hostname=registry.example.com" \
...
See: **[docs/MIRROR_REGISTRY_DEPLOYMENT.md](docs/MIRROR_REGISTRY_DEPLOYMENT.md)**

### Post-Installation

1. **Save credentials** from `/opt/mirror-registry/credentials.txt`
2. **Test health endpoint**: `curl -k https://registry.example.com:8443/health/instance`
3. **Login**: `podman login -u init registry.example.com:8443`
4. **Create pull-secret** for oc-mirror (see setup-registry-authentication.yml)
